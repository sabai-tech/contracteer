...
fun Map.Entry<String, MediaType>.mediaType() = value
fun Map.Entry<String, MediaType>.content() = key
fun Map<String, Header>.exampleKeys() = flatMap { it.value.safeExamples().keys }.toSet()
...
fun List<Parameter>.exampleKeys() = flatMap { it.safeExamples().keys }.toSet()
fun Content.exampleKeys() = flatMap { it.value.safeExamples().keys }.toSet()
fun ApiResponse.safeHeaders() = headers ?: emptyMap()
fun ApiResponse.exampleKeys() = safeHeaders().exampleKeys() + bodyExampleKeys()
fun ApiResponse.bodyExampleKeys() = content?.exampleKeys() ?: emptySet()
