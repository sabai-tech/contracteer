...
class FormStyleCodecTest {

  // ===== Encode =====

  @Test
  fun `encode primitive`() {
    assert(FormStyleCodec("color", explode = false).encode("blue") == listOf("color" to "blue"))
  }

  @Test
  fun `encode array with explode false`() {
    val result = FormStyleCodec("color", explode = false).encode(listOf("blue", "black", "brown"))
    assert(result == listOf("color" to "blue,black,brown"))
  }

  @Test
  fun `encode array with explode true`() {
    val result = FormStyleCodec("color", explode = true).encode(listOf("blue", "black", "brown"))
    assert(result == listOf("color" to "blue", "color" to "black", "color" to "brown"))
  }

  @Test
  fun `encode object with explode false`() {
    val result = FormStyleCodec("color", explode = false).encode(mapOf("R" to 100, "G" to 200, "B" to 150))
    assert(result == listOf("color" to "R,100,G,200,B,150"))
  }

  @Test
  fun `encode object with explode true`() {
    val result = FormStyleCodec("color", explode = true).encode(mapOf("R" to 100, "G" to 200, "B" to 150))
  ...
  fun `decode primitive`() {
    ...
    // when
    val result = FormStyleCodec("color", explode = false).decode(extractor, stringDataType())
  ...
  fun `decode array with explode false`() {
    ...
    // when
    val result = FormStyleCodec("color", explode = false).decode(extractor, arrayDataType(itemDataType = stringDataType()))
  ...
  fun `decode array with explode true`() {
    ...
    // when
    val result = FormStyleCodec("color", explode = true).decode(extractor, arrayDataType(itemDataType = stringDataType()))
  ...
  fun `decode object with explode false`() {
    ...
    // when
    val result = FormStyleCodec("color", explode = false).decode(extractor, rgbObjectDataType())
  ...
  fun `decode object with explode true`() {
    ...
    // when
    val result = FormStyleCodec("color", explode = true).decode(extractor, rgbObjectDataType())
  ...
  fun `decode returns null when value is absent`() {
    // when
    val result = FormStyleCodec("color", explode = false).decode(valueExtractor(), stringDataType())
