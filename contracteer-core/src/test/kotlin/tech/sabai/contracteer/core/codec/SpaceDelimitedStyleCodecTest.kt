...
class SpaceDelimitedStyleCodecTest {

  @Test
  fun `encode array`() {
    val result = SpaceDelimitedStyleCodec("color").encode(listOf("blue", "black", "brown"))
  ...
  fun `decode array`() {
    ...
    // when
    val result = SpaceDelimitedStyleCodec("color").decode(extractor, arrayDataType(itemDataType = stringDataType()))
  ...
  fun `decode returns null when value is absent`() {
    // when
    val result = SpaceDelimitedStyleCodec("color").decode(valueExtractor(), arrayDataType(itemDataType = stringDataType()))
